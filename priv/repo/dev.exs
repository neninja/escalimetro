  ...
  def run do
    ...
    pizza_ballot =
      ensure_ballot!(owner_scope, event, %{
        title: "Pizza",
        description: "Qual sabor de pizza voce prefere?",
    ...
    horario_ballot =
      ensure_ballot!(owner_scope, event, %{
        title: "Horario",
        description: "O encontro deve comecar as 19h30?",
        kind: "yes_no_maybe",
    ...
    cast_option_votes!(event, participants.rei, pizza_ballot, [
      {"Basca", true, "Minha favorita para dividir."},
      {"Calabresa", false, nil},
      {"Quatro Queijos", false, nil}
    ])

    cast_option_votes!(event, participants.neni, pizza_ballot, [
      {"Catuperu", true, "Boa opcao para variar."},
      {"Basca", false, nil},
      {"Margarita", false, nil}
    ])

    cast_option_votes!(event, participants.vitor, pizza_ballot, [
      {"Quatro Queijos", true, "Agrada quase todo mundo."},
      {"Calabresa", false, nil},
      {"Portuguesa", false, nil}
    ])

    cast_option_votes!(event, participants.bia, pizza_ballot, [
      {"Vegetariana", true, "Prefiro sem carne."},
      {"Margarita", false, nil}
    ])

    cast_value_vote!(event, participants.rei, horario_ballot, "yes", "Funciona bem.")
    cast_value_vote!(event, participants.neni, horario_ballot, "maybe", "Depende do trabalho.")
    cast_value_vote!(event, participants.vitor, horario_ballot, "yes", nil)
    cast_value_vote!(event, participants.bia, horario_ballot, "no", "Chego tarde nesse dia.")

    IO.puts("Cenario dev criado: #{event.title}")
    IO.puts("Usuarios dev: admin, rei, neni e vitor usam a senha #{@password}")
  end
